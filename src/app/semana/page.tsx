import { redirect } from 'next/navigation';
import Link from 'next/link';
import { createSupabaseServer } from '@/lib/supabase/server';
import { computeWeeklyMetrics } from '@/lib/metrics';
import { weekStartStr } from '@/lib/time';
import { WeeklyView } from '@/app/components/WeeklyView';
import type { DayBlock, Grabacion, Prioridad } from '@/lib/types';

export const dynamic = 'force-dynamic';

export default async function SemanaPage() {
  const supabase = await createSupabaseServer();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) redirect('/login');

  const weekStart = weekStartStr();

  const [metrics, blocksR, grabsR, priosR] = await Promise.all([
    computeWeeklyMetrics(supabase, user.id),
    supabase.from('day_blocks').select('*').eq('user_id', user.id).order('orden'),
    supabase.from('grabaciones').select('*').eq('user_id', user.id).gte('fecha', weekStart).order('created_at', { ascending: false }),
    supabase.from('prioridades').select('*').eq('user_id', user.id).order('tier').order('orden'),
  ]);

  return (
    <div className="flex min-h-dvh flex-col bg-ink-950">
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-ink-800 bg-ink-950/90 px-4 pb-3 pt-3 backdrop-blur safe-top">
        <Link href="/" className="rounded-full border border-ink-700 px-3 py-1.5 text-sm text-slate-300 active:scale-95">
          ← Hoy
        </Link>
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-500">Semana del {weekStart}</p>
          <h1 className="text-lg font-bold">Revisión semanal</h1>
        </div>
      </header>

      <main className="flex-1 overflow-y-auto px-4 pb-16 pt-4">
        <WeeklyView
          metrics={metrics}
          blocks={(blocksR.data ?? []) as DayBlock[]}
          grabaciones={(grabsR.data ?? []) as Grabacion[]}
          prioridades={(priosR.data ?? []) as Prioridad[]}
        />
      </main>
    </div>
  );
}
